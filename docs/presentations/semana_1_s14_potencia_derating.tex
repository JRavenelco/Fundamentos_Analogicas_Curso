...
\begin{frame}{Preguntas de verificación}
  \begin{enumerate}
    ...
    \item ¿Por qué la nominal se reduce al subir la temperatura ambiente?
      \uncover<5->{\emph{(La nominal se define a una temperatura de referencia
      (p. ej. $70\,^\circ\text{C}$). Al subir la temperatura ambiente el componente tiene menos
      margen térmico: para no exceder su temperatura máxima debe disipar \textbf{menos},
      por eso se aplica derating. En el vacío del espacio no hay convección, así que el
      sobrecalentamiento es aún mayor.)}}
  \end{enumerate}
\end{frame}
